...
from newapt.utils import RED, BLUE, GREEN, YELLOW, ask, dprint

# Overriding apt cache so we can make it exit on ctrl+c
class newaptCache(apt.Cache):
...
class newaptProgress(text.AcquireProgress):
...
class newaptOpProgress(base.OpProgress, newaptProgress):
	...
	def __init__(self, outfile=None, verbose=False):
		newaptProgress.__init__(self, outfile, verbose)
