...
import json
import apt_pkg
from newapt.columnar import Columnar
from newapt.utils import dprint, iprint, logger_newline, ask, shell, RED, BLUE, YELLOW, GREEN
from newapt.progress import newaptCache, newaptOpProgress, newaptProgress, InstallProgress
...
NEWAPT_DIR = Path('/var/lib/newapt')
NEWAPT_HISTORY = Path('/var/lib/newapt/history')
...
class newapt:
	...
	def __init__(self,  download_only = False,
		...
		if not no_update:
			...
			newaptCache().update(newaptProgress(verbose=verbose))

		self.cache = newaptCache(newaptOpProgress(verbose=verbose))
		...
		self.essential = []
		self.newapt = []
		self.newapt_depends = ['newapt', 'python3-pyshell']
			...
			if pkg_name in self.cache:
				...
				if pkg.installed:
					if not pkg.essential:
						if not pkg.shortname in self.newapt_depends:
							pkg.mark_delete()
							dprint(f"Marked delete: {pkg.name}")
						else:
							self.newapt.append(pkg.name)
					else:
	...
	def history(self):
		if not NEWAPT_HISTORY.exists():
			print("No history exists..")
			return
		history = NEWAPT_HISTORY.read_text().splitlines()
	...
	def history_clear(self, id):
		...
		if id == 'all':
			...
			with open(NEWAPT_HISTORY, 'w') as file:
	...
	def _get_changes(self, upgrade=False, remove=False):
		...
		if upgrade and not pkgs:
			...
			print(style("Nothing for NewApt to do.", bold=True))
			exit(0)
		elif remove and not pkgs:
			print(style("Nothing for NewApt to remove.", bold=True))
			exit(0)
		if pkgs:
			...
			for pkg in pkgs:
				...
				elif pkg.shortname in self.newapt_depends and pkg.marked_delete:
					self.newapt.append(pkg.name)
		...
		if self.download_only:
			...
			if upgrade:
				self.cache.commit(
					newaptProgress(self.verbose, self.debug),
				...
				from newapt.progress import base
				self.cache.commit(
